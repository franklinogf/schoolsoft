$(document).ready(function () {
	...
	$(document).on("click", ".delLink", function (e) {
		...
		if ($(this).parent().children()[0].value.length > 0) {
			if (confirm(__LANG === 'es' ? '¿Seguro desea eliminar este link?' : 'Are you sure you want to delete this link?')) {
	...
	$("#addLink").click(function (e) {
		...
		<div class="form-group form-inline row linkGroup animated fadeInDown faster">
			<input class="form-control my-1 col-7" type="url" required name="link[]" placeholder="${__LANG === 'es' ? 'https://www.ejemplo.com' : 'https://www.example.com'}">
			<input class="form-control my-1 col-4" type="text" name="linkName[]" placeholder="${__LANG === 'es' ? 'Titulo (opcional)' : 'Title (Optional)'}">
	...
	$("#newMessageModal .continueBtn").click((e) => {
		...
		if (checked > 0) {
			...
			$("#newMessageModal .studentsAmount").text(
				`${__LANG === 'es' ? 'Este correo se le enviara a' : 'This email will be sent to'} ${checked} ${plural}`
			);
			animateCSS($studentsTableWrapper, "zoomOut faster", () => {
				$studentsTableWrapper.hide(0);
	...
	$newMessageModal.on("click", ".closeModal", function (e) {
		if (
			...
			if (confirm(__LANG === 'es' ? '¿Esta seguro que desea salir?' : 'Are you sure you want to exit?')) {
				$newMessageModal.modal("hide");
			}
		} else {
			$newMessageModal.modal("hide");
	...
	$($message).on("click", "#respondBtn", function (e) {
		...
		$respondModal.find(".modal-title").text(`${__LANG === 'es' ? 'Responder a' : 'Respond back to'} ${message.nombre}`);
	...
	$($messages).on("click", "div.card", function (e) {
      ...
      <div class="row">
         ...
         <div class="col-10">
            <div class="media p-2 mt-2">
               ...
               ${message.enviadoPor !== "p"
				? `<button id="respondBtn" title="${__LANG === 'es' ? 'Responder' : 'Respond'}" class="btn btn-secondary btn-sm" data-toggle="tooltip" type="button">
         ...
         <div class="col-2 d-flex justify-content-center align-items-center">
            <p class="m-0">${__LANG === 'es' ? 'Para' : 'To'}:</p>
         </div>
         <div class="col-10">
            ${message.to.length === 1
				? `
            ...
            `
				: `${__LANG === 'es' ? `Enviado a ${message.to.length} estudiantes` : `Sent to ${message.to.length} students`} <a id="viewStudents" href="#" class="badge badge-primary">${__LANG === 'es' ? 'Ver' : 'View'}</a>`
			}
         </div>
      </div>
      ...
      <div class="modal fade" id="viewStudentsModal" data-backdrop="static" tabindex="-1" role="dialog" aria-hidden="true">
         <div class="modal-dialog" role="document">
            <div class="modal-content">
               ...
               <h5 class="modal-title">${__LANG === 'es' ? 'Lista de estudiantes del grado' : 'Students list of the grade'} <span class="badge badge-secondary">${message.to[0].info}</span> </h5>
               ...
               <div class="modal-footer">
                  <button type="button" class="btn btn-secondary" data-dismiss="modal">${__LANG === 'es' ? 'Cerrar' : 'Close'}</button>               
               </div>
            </div>
         </div>
      </div>
		...
		$.post(
			...
			async (res) => {
               ...
               <div class="card-body p-2">
                  <p class="card-text mb-0 font-weight-bold">${type === 'inbound' ? message.nombre : message.to.length > 1 ? `${__LANG === 'es' ? 'Enviado a' : 'Sent to'} ${message.to.length} ${__LANG === 'es' ? 'estudiantes' : 'students'}` : message.to[0].nombre}</p>
