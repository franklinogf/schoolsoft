...
$lang = new Lang([
  ['Botones de la cafeteria', 'Cafeteria buttons'],
  ['Agregar Boton', 'Add button'],
  ['Eliminar', 'Delete'],
  ['Guardar', 'Save'],
  ['Editar Boton', 'Edit button'],
  ['Titulo', 'Title'],
  ['Precio:', 'Price:'],
  ['No imagen', 'No image']
]);

$result = DB::table('T_cafeteria')->orderBy('orden')->get();
...
<head>
  <?php
  $title = $lang->translation("Botones de la cafeteria");
  ...
  <div class="container">
    ...
    <div class="row">
      ...
      <div id="sortable" class="col-md-8 order-md-1 bg-white shadow">
        <?php foreach ($foods as $food): ?>
          <?php foreach ($food as $foo): ?>
            <div id="<?= $foo->id ?>" class="card" data-action='edit' data-target="#Modal" data-toggle="modal">
              <a href="#"><img class="card-img-top" src="<?= isset($foo->foto) ? "../../../cafeteria_im/$foo->foto" : '../../../cafeteria_im/no-image.png' ?>" alt="<?= $foo->foto ?>"></a>
              <div class="card-body">
                <p class="card-title"><?= $foo->articulo ?></p>
                <span class="price"><b><?= $foo->precio ?></b></span>
              </div>
            </div>
          <?php endforeach ?>
      ...
      <div class="col-md-4 order-md-2 mb-4 ">
        <a href="#" id="add" class="btn btn-primary btn-lg btn-block <?= ($cant_buttons == $max_buttons) ? 'disabled' : '' ?>" data-action='add' data-toggle="modal" data-target="#Modal">Agregar Boton <i class="fas fa-plus"></i></a>
              ...
              <div class="modal-body">
                ...
                <div id="images">
                  <?php foreach ($images as $image): ?>
                    <img src="<?= $image ?>" alt="<?= substr(strrchr($image, '/'), 1) ?>" class="img-thumbnail">
                  <?php endforeach ?>
                </div>
