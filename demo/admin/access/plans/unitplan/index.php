...
if ($teacherId) {
    $teacher = Teacher::with('unitPlans')->find($teacherId);
    if (!$teacher) {
        die('Profesor no encontrado');
    }
    $plans = $teacher->unitPlans;
}
...
<body>
    ...
    <div class="container-md mt-md-3 mb-md-5 px-0">
        <h1 class="text-center my-3"><?= __("Plan de unidad") ?></h1>
        <div class="container bg-white shadow-lg py-3 rounded">
            ...
            <?php if ($teacher) : ?>
                <h2 class="text-center my-4"><?= __("Planes de unidad para") . " $teacher->apellidos $teacher->nombre" ?></h2>
                            ...
                            <tbody>
                                <?php foreach ($plans as $plan) : ?>
                                    <tr>
                                        <td><?= $plan->materia ?></td>
                                        <td><?= $plan->titulo ?></td>
                                        <td><a target="_blank" href="<?= Route::url("/admin/access/plans/unitplan/pdf.php?id={$plan->{$plan->getKeyName()}}") ?>" class="btn btn-sm btn-primary"><?= __("Imprimir") ?></ata>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                ...
                <?php else : ?>
                    <p class="text-center"><?= __("No se encontraron planes de unidad para este profesor.") ?></p>
                <?php endif; ?>
