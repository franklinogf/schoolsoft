...
if ($teacherId) {
    $teacher = Teacher::with('classPlans')->find($teacherId);
    if (!$teacher) {
        die('Profesor no encontrado');
    }
    $plans = $teacher->classPlans;
}
...
<body>
    ...
    <div class="container-md mt-md-3 mb-md-5 px-0">
        <h1 class="text-center my-3"><?= __("Plan de clase") ?></h1>
        <div class="container bg-white shadow-lg py-3 rounded">
            ...
            <?php if ($teacher) : ?>
                <h2 class="text-center my-4"><?= __("Planes de clases para") . " $teacher->apellidos $teacher->nombre" ?></h2>
                            ...
                            <tbody>
                                <?php foreach ($plans as $plan) : ?>
                                    <tr>
                                        <td><?= $plan->materia ?></td>
                                        <td><?= $plan->tema ?></td>
                                        <td><a target="_blank" href="<?= Route::url("/admin/access/plans/classplan/pdf.php?id={$plan->{$plan->getKeyName()}}") ?>" class="btn btn-sm btn-primary"><?= __("Imprimir") ?></ata>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                ...
                <?php else : ?>
                    <p class="text-center"><?= __("No se encontraron planes de clases para este profesor.") ?></p>
                <?php endif; ?>
