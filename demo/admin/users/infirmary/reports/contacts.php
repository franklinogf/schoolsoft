...
use App\Pdfs\Infirmary\ContactsReportPdf;
use Classes\Route;
use Classes\Session;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($studentId) {
        $student = Student::with(['family', 'infirmary'])->find($studentId);

        if ($student) {
            $pdf = new ContactsReportPdf($student);
            $pdf->generate();
            $pdf->Output();
            exit;
        }
...
<head>
    <?php
    $title = __("Contactos de Enfermería");
