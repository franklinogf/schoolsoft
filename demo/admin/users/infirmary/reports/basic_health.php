...
use App\Pdfs\Infirmary\BasicHealthReportPdf;
use Classes\Route;
use Classes\Session;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($studentId) {
        $student = Student::with(['infirmary', 'family'])->find($studentId);

        if ($student) {
            $pdf = new BasicHealthReportPdf($student);
            $pdf->generate();
            $pdf->Output();
            exit;
        }
...
<head>
    <?php
    $title = __("Informe de Enfermería");
