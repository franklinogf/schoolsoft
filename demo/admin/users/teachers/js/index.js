$(document).ready(function () {
  $(".expire").change(function () {
    ...
    $(this).parent().parent().find(".expireDate").prop("disabled", !checked);
  });

  $("#type").change(function () {
    console.log("Type Changed");
    if ($("#type").val() === "Docente") {
      $("#level,#home,#club input").prop("disabled", false);
    } else {
      $("#level,#home,#club input").prop("disabled", true);
    }
  });
  ...
  $("#username").keyup(function (e) {
    if ($("#username").val().length > 0) {
      if (
        $("#username").data("lastusername").toString() !== $("#username").val()
      ) {
        if (!searched) {
          $.post(
            includeThisFile(),
