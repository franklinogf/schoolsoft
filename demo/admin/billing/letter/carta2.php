...
foreach ($paymentGroup as $id => $payments) {
    ...
    $pdf->AddPage();
    $pdf->Cell(0, 5, __('Segundo aviso de cobro'), 0, 1);
    $pdf->Ln(5);

    $pdf->Cell(0, 5, $todayFormatted, 0, 1);
    $pdf->Ln(5);
    $pdf->Cell(0, 5, __('Estudiantes'), 0, 1);
    $pdf->Ln(3);
    ...
    $pdf->Ln(3);
    $filePath = "{$directory}/letter2_{$id}.pdf";
...
if ($_POST['tipo'] === 'email') {
    foreach ($files as $familyId => $file) {
        ...
        Email::to($to)
            ->subject(__('Segundo aviso de cobro'))
            ->body(__('Adjunto el segundo aviso de cobro para la cuenta #') . $familyId)
