$(document).ready(function () {
  ...
  $('#latePaymentForm').submit(function (e) {
    ...
    if (observationType || alert) {
      $('#latePaymentButton').addClass('btn-danger').removeClass('btn-secondary')
    } else {
      $('#latePaymentButton').removeClass('btn-danger').addClass('btn-secondary')
    }
  })
  ...
  $('#depositForm').submit(function (e) {
    ...
    if ((type === 10 || type === 11) && (date.length <= 0 || time.length <= 0)) {
      $('#depositDate,#depositTime').addClass('is-invalid')
      return
    }
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.error) {
          ...
          $('#editChargeCode').val(response.code)
  ...
  $('#paymentMode').change(function (event) {
    ...
    if (value === 'parcial') {
      ...
      $(`#table${_month} tr`).each(function (index, tr) {
        ...
        const ss = tr.dataset.ss
        const grade = tr.dataset.grade
        const desc = tr.dataset.description
        const debt = parseFloat(tr.dataset.debt)
        const paid = parseFloat(tr.dataset.payment)
        ...
        if (!debts[`${id}-${ss}`]) {
          ...
          debts = {
            ...
            [`${id}-${ss}`]: {
              ...debts[`${id}-${ss}`],
              debt: debts[`${id}-${ss}`].debt + debt,
              paid: debts[`${id}-${ss}`].paid + paid
            }
          }
        }
      })
  ...
  function addChargeToTable(monthToAdd, { id, grade, description, date, debt, mt }) {
    const tr = `
