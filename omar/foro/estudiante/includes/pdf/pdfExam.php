...
$pdf = new PDF();
$pdf->SetTitle(utf8_decode(utf8_decode($exam->titulo)) . " - " . $student->fullName());
$pdf->AddPage();
$pdf->SetAutoPageBreak(true, 10);

// Util::dump($exam);
$points = $doneExam->puntos + $doneExam->bono;

$pdf->SetFont('Arial', 'B', 12);
$pdf->Cell(20, 7, "Nombre: ", 0, 0, "L");
$pdf->Cell(80, 7, $student->fullName(), 'B', 0, 'L');
$pdf->Cell(25, 7, '', 0, 0, 'L');
$pdf->Cell(18, 7, 'Fecha: ', 0, 0, 'L');
$pdf->Cell(35, 7, Util::formatDate($doneExam->terminado_el, true, true), 'B', 1, 'C');
$pdf->Ln(5);
$pdf->Cell(100, 7, utf8_decode($teacher->fullName()), 0, 0, "L");
$pdf->Cell(25, 7, '', 0, 0, 'L');
	...
	foreach ($exam->selects->topics as $topic) {
		...
		for ($i = 1; $i <= 8; $i++) {
			if (!empty($topic->{"respuesta{$i}"})) {
				...
				$pdf->Cell(0, 5, utf8_decode($topic->{"respuesta{$i}"}), 0, 1);
			}
			black();
		}
		$pdf->Ln(5);
...
if (isset($exam->pairs->topics)) {
	...
	foreach ($exam->pairs->topics as $topic) {
		...
		$pdf->MultiCell(90, 5, utf8_decode($topic->pregunta), 0, 1);
		$count++;
	}
	black();
	$pdf->SetY($Y);
	foreach ($exam->pairCodes->topics as $index => $answer) {
		$pdf->Cell(110);
		$pdf->MultiCell(80, 5, $lettersArray[$answer->id] . ') ' . utf8_decode($answer->respuesta));
	}
	$topicNumber++;
}
if (isset($exam->lines->topics)) {
	...
	foreach ($exam->lines->topics as $index => $topic) {
		...
		foreach ($words as $word) {
			if ($word !== '~') {
				$pdf->Cell($pdf->GetStringWidth($word) + 1, 5, utf8_decode($word));
			} else {
				...
				$pdf->Cell($pdf->GetStringWidth($done->{"respuesta{$answerNumber}"}) + 2, 5, utf8_decode($done->{"respuesta{$answerNumber}"}), 'B');
			}
			black();
		}
		$count++;
