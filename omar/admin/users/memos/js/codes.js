        ...
        if (code !== '' && description !== '') {
            ...
            if ($(this).data('option') === 'add') {
                ...
                <div id="${id}" class="card h-100">
                    <div class="card-body">
                        <p class="card-text float-right"><span class="badge badge-info value">${value}</span></p>
                    ...
                    <div class="card-footer text-center">
                        <button class="btn btn-primary edit" data-id="${id}">${__LANG === 'es' ? 'Editar' : 'Edit'}</button>
                        <button class="btn btn-danger del" data-id="${id}">${__LANG === 'es' ? 'Eliminar' : 'Delete'}</button>
                    </div>
                </div>
            </div>`)
                    }
