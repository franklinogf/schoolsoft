$(document).ready(function () {
  ...
  $("#delete").click(function (e) {
    if ($("#new option:selected").length > 0) {
      ...
      if (students.length > 1) {
        msg =
          __LANG === "es"
            ? "Esta seguro que quiere borrar estos estudiantes?"
            : "Are you sure you want to delete this student?";
      } else {
        msg =
          __LANG === "es"
            ? "Esta seguro que quiere borrar este estudiante?"
  ...
  $("#deleteAll").click(function (e) {
    if (
      confirm(
        __LANG === "es"
          ? "Esta seguro que quiere borrar todos los estudiantes?"
  ...
  $("#pass").click(function (e) {
    ...
    if ($("#old option:selected").length > 0) {
      ...
      $.post(includeThisFile(), jsonData, function (data, textStatus, jqXHR) {
        if ($("#newGrade").val() !== "") {
          searchGradeStudents("new", $("#newGrade").val(), {
            new: $("#year2").val(),
          });
        } else {
          searchAllStudents("new");
        }
        $("#oldStudentsAmount").text(
          +$("#oldStudentsAmount").text() - passStudents.length
        );
  ...
  $("#studentSurnames").keyup(function (e) {
    if ($("#studentSurnames").val().length > 0) {
      ...
      if ($("#oldGrade").val()) {
        ...
        $("#pass").html(
          `${
            __LANG === "es" ? "Pasar a grado" : "Pass to grade"
          } <i class="fas fa-angle-double-right d-none d-lg-block"></i> <i class="fas fa-angle-double-down d-lg-none"></i>`
        );
  ...
  const searchAllStudents = (type) => {
    ...
    $(`#${type}StudentsTitle`).html(
      `${
        __LANG === "es" ? "Todos los estudiantes" : "All the students"
      } <span id="${type}StudentsAmount" class="badge text-bg-primary"></span>`
    );
  };
  const searchStudentsBySurnames = (jsonData) => {
    searchStudents("old", jsonData);
    $(`#oldStudentsTitle`).html(
      `${
        __LANG === "es"
          ? "Todos los estudiantes que contiene en el apellido"
          : "All the students that contains on the last names"
      } <b>\"${
        jsonData.searchBySurname
      }\"</b> <span id="oldStudentsAmount" class="badge text-bg-primary"></span>`
    );
  };
    ...
    $(`#${type}StudentsTitle`).html(
      `${
        __LANG === "es"
          ...
          : "All the students in grade"
      } ${grade} <span id="${type}StudentsAmount" class="badge text-bg-primary"></span>`
    );
  };
});
