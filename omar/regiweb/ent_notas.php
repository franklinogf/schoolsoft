      ...
      var countdowntimer = {
         ...
         update: function() {
            ...
            if (countdowntimer.seconds == 0 && se == 0) {
               ...
               document.getElementById("sound_element").innerHTML =
                  "<embed src='../../jv/alerta.mp3' hidden=true autostart=true loop=false>";
               setTimeout("alert('El tiempo est� llegando al final, grabe antes que pierda la informaci�n o ya es tarde..')", 3000);
               se = 1;
            } else {
...
date_default_timezone_set("America/puerto_rico");
  ...
  if ($_POST[tri] == "Selecci�n" OR $_POST[tra] == "Selecci�n")
  {
     echo "<CENTER>No has seleccionado las opciones para la entradas de notas.<br>Por favor vuelve e int�ntalo de nuevo.";
...
echo		'<td bgcolor="#FFFFCC"><center>Cuenta atr�s</center></td>';
echo	'</tr>';
echo	'<tr>';
...
if ($_POST[tra] == "Notas" OR $_POST[tra] == "V-Nota")
   ...
   if ($_POST[tra] == "Notas")
      {
      echo '<td>Est� opci�n se aplica en la columna <b>"Nota-9"</b> exclusivamente.</td>';
      }
   ELSE
      {
      echo '<td>Est� opci�n se aplica en la columna <b>"Nota-7"</b> exclusivamente.</td>';
      }
   echo	'</tr>';
   echo '</table>';
   ...
   echo '<br><table border="0" cellpadding="2" cellspacing="0" width="50%">';
   echo	'<tr>';
   echo		"<td width='170' align='right' bgcolor='#CCCCCC'><b>Conversi�n </b>";

   $chk1=""; IF($reg[105]=="ON") {$chk1="checked=checked";}
   echo "<input name=pal type='checkbox' value='ON' ".$chk1.'/>';

   echo		'</td>';
   echo		'<td>Est� opci�n es para convertir de numero a letra.</td>';
   echo	'</tr>';
   echo '</table>';
if ($_POST['tri'] == "Trimestre-2" and $reg6[61]=="NO" OR $_POST['tri'] == "Trimestre-4" and $reg6[61]=="NO")
   ...
   echo		'</td>';
   echo		'<td>Est� opci�n suma T-1+T-2 / T-3+T-4.</td>';
   echo	'</tr>';
   echo '</table>';
   }
}
...
if ($_POST[tri] == "Trimestre-1")
   ...
   echo		'</td>';
   echo		'<td>Cuando termine el trimestre marque est� Opci�n.</td>';
   echo	'</tr>';
   echo '</table>';
   }
...
if ($_POST[tra] == "Cond-Asis")
   ...
   echo		'<td width="50" align="center" bgcolor="#CCCCCC"><b>Dem�ritos</b></td>';
   if ($reg6[87] == "SI")
      {
...
if ($_POST[tri] == "Trimestre-1")
   ...
   if (date("Y-m-d") < $reg6[30] or $reg2[93]==1 and $reg2[94]==1 or $reg2[93]==1 and $reg2[94]==5)
      ...
      echo 'Selecci�n del trimestre equivocada.</font></b></p>';
      echo '<p align="center"><b><font size="5">Intentelo de Nuevo o Comuniquese con la ';
      echo 'Administraci�n.</font></b></p>';
      }
   }
...
<script>
   // Al presionar cualquier tecla en cualquier campo de texto, ejectuamos la siguiente funci�n
   $('input').on('keydown', function(e) {
      // Solo nos importa si la tecla presionada fue ENTER... (Para ver el c�digo de otras teclas: http://www.webonweboff.com/tips/js/event_key_codes.aspx)
      if (e.keyCode === 13 || e.keyCode === 40) {
         // Obtenemos el n�mero del tabindex del campo actual
         ...
         // Si no se encontro ning�n elemento, no hacemos nada (se envia el formulario)
      }
      if (e.keyCode === 38) {
         var currentTabIndex = $(this).attr('tabindex');
         // Le sumamos 1 :P
